...
- **`background.js`**  
  확장 프로그램의 중심에서 메시지를 중계하고, 상태 알람 작업을 수행합니다.

- **`filtered.html` / `filtered.js`**  
  필터링된 채팅만 보여 주는 팝업 창입니다. 백그라운드 스크립트와 연결하여, 전송받은 메시지를 화면에 표시합니다.

- **`popup.html` / `popup.js`**  
  크롬 확장 프로그램 아이콘을 클릭했을 때 열리는 팝업 인터페이스를 구성합니다. “채팅 걸러내기” 버튼으로 `filtered.html` 창을 띄울 수 있습니다.

- **`manifest.json`**  
  확장 프로그램의 설정(권한, 버전, 실행 스크립트 등)를 선언하는 파일입니다.
